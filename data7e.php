    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="data7.php" class="nav-link active">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Administrative Officers</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
<?php
  ...
  $user_check_query = "SELECT * FROM tab2 WHERE fname='$firstName' AND mname='$midName' AND lname='$lastName' AND position='$course' AND year='$year' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  $user = mysqli_fetch_assoc($result);

  if ($user) { // if user exists
    if ($user['fname'] === $firstName AND $user['mname'] === $midName AND $user['lname'] === $lastName AND $user['year'] === $year) {
       echo "<script>alert('Member detail already exists.');window.location='data7e.php';</script>";
    }
  }
  else{
         ...
         $result = mysqli_query($db, $user_check_query);
         echo "<script>window.location='data7.php';alert('Added Successfully!');</script>";
       }
      }
?>